...
### Run 20260805-sms-ratelimit — SMS resend rate-limit (429) (2026-08-05)
- **What worked**: closed the SMS-cost/bombing vector with a MIGRATION-FREE rate limit — the last-send instant is derived as `sms_challenge_expires_at - SMS_CHALLENGE_TTL` from the EXISTING column, so no schema change and `check_migration` stayed green (146==146). One shared `_enforce_sms_resend_interval()` helper guards BOTH send paths (`_prepare_sms` re-arm + `MfaChallengeService.issue`), so the vector can't be reopened via the other endpoint. `SMS_RESEND_INTERVAL=60s < SMS_CHALLENGE_TTL=300s` by construction.
- **What the independent reviewer caught**: no findings at any severity — but the value here was independent confirmation of a *no-op* claim. T1 said it did NOT modify the OpenAPI file because 429 was "already declared" on both ops; the reviewer OPENED `ep1-identity.yaml` and confirmed `429 -> TooManyRequests -> Error` names `RATE_LIMITED` on both `createMfaEnrollment` and `createMfaChallenge`. It also re-derived the post-expiry boundary (strict `<`, naive-UTC both sides — no off-by-one, no aware/naive subtraction bug) and confirmed the 429 path calls send ZERO times (test asserts `len(sent)==1`), plus 423-lockout-before-429 ordering. "Contract already covers it" is exactly the class of unverified assertion this project has been burned by; probing it is the point.
- **Claims marked UNVERIFIED**: none material.
- **New knowledge**: (1) A per-factor rate limit spanning N endpoints will break any existing test that chains those endpoints on ONE factor inside the window — here `test_stepup.py`'s enroll->immediately-re-challenge reissue test. The coder correctly STOPPED at its file-scope boundary and reported the cross-file breakage rather than editing an out-of-scope test; the orchestrator then expanded scope on the same branch. Fix the test by AGEING the timestamp past the cooldown (mirroring the new tests) — never weaken the limit or bump the interval to make a test pass. (2) A retry-model-upgrade was NOT applied for the scope expansion because the coder didn't fail — it hit a declared boundary.
- **Planning advice**: when a task adds a rate limit / shared guard across endpoints that touch one resource, pre-plan the `files_hint` to INCLUDE the tests that already chain those endpoints (grep the test dir for both route paths before finalising), so the fix lands in one scoped task instead of a scope expansion.
- **Verifier**: HARD 5/5 (all 0) · DRIFT USD 117 / rails 1 / vendor 0 = baseline · not re-baselined (code-only change) · 150 tests.
- **Backlog carried forward**: 11 items unchanged; the SMS rate-limit item is now CLOSED (was 20260803 R4 INFO/low).
### Run 20260805-ep2-readiness — EP-2 share-purchase build-readiness plan (2026-08-05)
- **What worked**: scouting BEFORE planning changed the task — a grep showed the EP-2 contract (`ep2-shares.yaml`) and the ledger/membership schema already exist, so the deliverable became a *verified readiness/implementation plan* (`docs/ep2-share-purchase-readiness.md`), not new contract code. The plan's core value is THE SPLIT: ratification-independent work (par-check `422 AMOUNT_MISMATCH`, Idempotency-Key plumbing, DEC-4 status guard, payment port+mock, the read GETs) vs ledger-gated work (the `SHARE_SUBSCRIPTION` posting, `PENDING_PAYMENT→ACTIVE` activation, L-4) — so the eventual build can start without touching `06`.
- **What the independent reviewer caught**: no false claims and — the load-bearing check — the split is SAFE (re-derived every "buildable now" item; none writes a LedgerEntry / posts / reads a derived balance / flips to ACTIVE; the doc correctly does NOT apply `require_active_member` to the purchase endpoint, which would block a legit PENDING_PAYMENT buyer). Only two mechanical MICRO-FIXes (a 10-vs-9 op overcount; a missing `06:120` citation for account 5400). The value of review on a *planning* doc is confirming the classification is safe to build against — a mis-split that green-lit implementing a `06` posting pre-ratification would have been a non-negotiable breach.
- **Claims marked UNVERIFIED**: the analyst left the `ShareStatus.MEMBER = "ISSUED"` enum name/value runtime consequence `[UNVERIFIED]` rather than execute code to test SQLAlchemy serialization — correct call for a no-code task; the reviewer confirmed the source-level mismatch is real and the runtime question genuinely open. Carried as a gap for the EP-2 build.
- **New knowledge**: (1) `require_active_member` (`auth/deps.py:132-157`) already implements suspended→uniform-403-no-leakage and is unwired — EP-2 reuses it, but NOT on the purchase endpoint. (2) Real cross-doc gaps surfaced for follow-up: `05:53` claims `03`'s EP-2 covers `422 AMOUNT_MISMATCH` but the string is absent in `03` (G-3); `03` US-2.1 Scenario 4 premises a `PENDING_KYC` authenticated client, a state `kyc.py` makes transient/uncommitted (G-4). (3) The `1010/1000/5400/3000` chart-of-accounts and `Account` fields (`system_account_code`/`account_category`/`normal_balance`) do NOT exist in `ledger.py` yet — the schema amendment is itself part of what U1 must bless.
- **Planning advice**: for any "build epic X" ask, grep the contract + models FIRST — several epics may already have their contract/schema scaffolded (the 61-table schema + 192-op contract are broad), so the real work is wiring + a gated subset, not greenfield. Scope the task to the gap, not the whole epic.
- **Verifier**: HARD 5/5 (all 0) · DRIFT USD 117 / rails 1 / vendor 0 = baseline · not re-baselined (docs-only) · verify-docs PASS (the `docs/` file is outside its scanned set).
- **Backlog carried forward**: 11 items; the currency-completeness sweep item marked closed for migrated surfaces (this session); ADDED the EP-2 gap set (G-1..G-6, see the readiness doc) as a build-time follow-up.
### Run 20260806-ep2-gap-fixes — EP-2 readiness gap fixes (2026-08-06)
- **What worked**: the prior run's readiness review produced a concrete gap list (G-1..G-6); this run fixed the safe, non-U1-gated ones and correctly routed the rest. Doc fixes (T1, opus spec_writer): G-3 added a genuinely MISSING money-critical acceptance scenario (US-2.1 settlement amount-mismatch → `422 AMOUNT_MISMATCH`, verified distinct from the request-side 422 — the settlement code is on the provider-authenticated `paymentsWebhook`, not member-facing); G-4 reframed an architecturally-unreachable security premise (a `PENDING_KYC` app can't present member auth) to a reachable one; G-5 aligned an illegal-transition code to the contract (`409`→`422 ILLEGAL_TRANSITION`; the contract's `409` there is `PENDING_TRANSITION_EXISTS`). Recount 5+4+4+4=17, `05` updated 16→17.
- **What the independent reviewer caught**: T2 (opus) resolved the 403-vs-409 question the spec_writer flagged — the share-purchase op declares `409 WRONG_MEMBERSHIP_STATE` for exactly "member not in PENDING_PAYMENT", so the reframed scenario's `403` was itself a G-5-class AC-vs-contract mismatch. Applied as a micro-fix.
- **Orchestrator adjudication (the non-obvious part)**: the micro-fix isn't fully clean — `03`'s own line-13 HTTP-semantics convention says status-check-fail → `403` and reserves `409` for duplicate/replay conflicts, so `409 WRONG_MEMBERSHIP_STATE` (and G-5's `422 ILLEGAL_TRANSITION`) both technically diverge from `03`'s loose convention. Resolved by the consistent policy **AC follows the contract's declared per-op codes** (the executable source of truth), and FLAGGED that `03` line-13 needs a broader convention-vs-contract reconciliation as a follow-up rather than pretending the micro-fix closed it. Lesson: when aligning AC to a contract code, check the doc's own HTTP-semantics convention — a per-scenario code swap can silently contradict it.
- **What T3 found (honest block)**: G-6 (`ShareStatus.MEMBER = "ISSUED"`) was scoped as a value-preserving cosmetic rename, but `Enum(..., name="share_status")` has NO `values_callable`, so SQLAlchemy persists on `.name` — the DB actually stores `'MEMBER'`, disagreeing with `04:181` + contract (`'ISSUED'`). So `.value="ISSUED"` is dead weight and the real fix needs a MIGRATION (rename the PG enum value + Python member + add values_callable). T3 correctly reverted (stayed in its no-migration boundary) and reclassified. This upgrades readiness-doc G-6 from `[UNVERIFIED]` to `[VERIFIED]`. DEFERRED to the EP-2 schema-amendment migration (bundle with G-1/G-2). Lesson: a "cosmetic enum rename" is only cosmetic if `values_callable` makes name and value independent — without it, the Python name IS the wire value and any rename is a schema change.
- **Claims marked UNVERIFIED**: none outstanding; T3 turned the one prior `[UNVERIFIED]` (G-6 runtime) into `[VERIFIED]`.
- **Planning advice**: when a gap is "an enum name/value mismatch", check `values_callable` FIRST to know whether it's a doc/cosmetic fix or a migration — it changes the task type (coder-no-migration vs migration-serialized).
- **Verifier**: HARD 5/5 (all 0) · DRIFT USD 117 / rails 1 / vendor 0 = baseline · not re-baselined · docs-only (T1) + no-op (T3); pytest stayed 150 on the T3 baseline.
- **Backlog carried forward**: G-3/G-4/G-5 CLOSED; G-6 reclassified to VERIFIED schema bug, deferred to the EP-2 schema-amendment migration with G-1/G-2; NEW follow-up — reconcile `03` line-13 HTTP-semantics convention with the contract's specific codes (409 WRONG_MEMBERSHIP_STATE / 422 ILLEGAL_TRANSITION) across all `03` status scenarios.
