...
**The application is at the SCAFFOLD stage.** As of 2026-07-22 a backend skeleton exists at `backend/` — FastAPI + Postgres, a `/health` and `/ready` endpoint, and the money non-negotiable encoded and tested (`backend/app/money.py`). **EP-1 onboarding vertical exists** (pre-auth draft → identity capture → common-bond eligibility → ХУР/XYP KYC → promotion to a Member at PENDING_PAYMENT), on a persistence→service→router foundation (repositories/, services/, api/routers/, adapters/ekyc/). Routes: `POST/GET/PATCH /api/v1/onboarding/applications*`, `/eligibility-check`, `/kyc/sessions`, `/kyc/status`, plus the **full EP-1 identity epic**: profile (`GET /members/me`, `PATCH …/me/profile`), consents (`GET/PUT …/me/consents`, append-only audit), devices (`GET /auth/devices`, `DELETE …/{id}` step-up-gated), and **MFA + step-up** (`POST /auth/mfa/enrollments` — TOTP secret encrypted at rest via Fernet + pyotp, SMS behind a mock port; `POST /auth/step-up` — mints a single-use, member-bound, 5-min opaque step-up token, hash-only, 423 lockout; `require_step_up` consumes it atomically). All member endpoints behind `get_current_member` (verified external-IdP RS256 Bearer JWT) (verifies external-IdP RS256 Bearer JWTs — the IdP is operator-chosen/self-host; the backend only VERIFIES, security-hardened against alg:none/HS256-confusion/expiry). KYC runs behind a provider PORT + MOCK (no real ХУР API in dev). DEC-4 enforced (a rejected KYC creates no Member). A member can be onboarded → KYC-approved → promoted (PENDING_PAYMENT) → authenticate → manage their own profile (IDOR-safe). It STOPS at PENDING_PAYMENT — NO share purchase / money / ledger / account / payment / governance routes (share purchase needs the ledger). The OpenAPI contract (`backend/openapi/`, 192 operations) and the ORM schema + initial migration (`backend/app/models/`, 61 tables, derived from `04_technical_architecture.md`) are built and gated (check_models, check_migration, openapi validate, pytest). Stack decision: `docs/adr/0001-technology-stack.md`.
...
- **`06_ledger_addendum.md` was independently audited (2026-07-30, see `07_ledger_audit_verdict.md`) and corrected in draft (2026-08-03).** The audit re-derived the design through three lenses and found the accounting model **verified sound** except for one disguised money-loss defect — the inverted hold formula (it computed available as `balance + hold`; the earlier `balance − signed_sum(MEMO_HOLD)` was cosmetic and still added the hold). That fix (`available = balance + signed_sum(MEMO_HOLD)`, with the `03:340 = 50,000₮` regression) plus the full Mongolia migration (par 10,000₮; ACH+/Banksüljee(RTGS)/NETC; `RTGS_OUT` posting rule; `9030` payment-hold memo account; reconciled to `04` E-8) and the LA-11 rate-vs-volume aggregation encoding are **now applied to `06` (see §11 changelog)**. **It remains do-not-implement pending CONTROLLER ratification** — a controller/core-banking engineer must sign off the design, not another LLM pass. The **dividend engine stays gated** on LA-8 (surplus split, Board), LA-11's remaining half (E-31 absent-value, PO+Finance), and LA-13 (eligibility, Counsel+PO); LA-14 (deposit negative-balance collections) should close before cards. Once ratified, share purchase (EP-2/US-2.1 `PENDING_PAYMENT→ACTIVE`) and the downstream money paths unblock.
