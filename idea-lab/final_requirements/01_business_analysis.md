...
│   ├── CAP-1.2  eKYC & Identity Verification (document OCR, liveness/selfie match, screening — provider is a procurement decision, TBD; see DEC-5)
...
│   ├── CAP-2.1  Membership Share Account (non-withdrawable equity; one initial share at 10,000₮ (provisional, pending DEC-11 / legal) par — see DEC-11)
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| F-101 | Instant Digital Onboarding & eKYC | CAP-1.1, CAP-1.2, CAP-1.3 | P-1, P-2, P-3 | Mobile application flow with document capture (OCR), biometric selfie match, and automated sanctions/PEP screening via the eKYC / identity-verification provider (document OCR, liveness, screening; provider is a procurement decision, TBD — see DEC-5); save-and-resume; manual-review fallback routed to CAP-12.2. Target: median ≤ 8 minutes end-to-end (KPI-1.1). | MVP |
...
| **DEC-5** | **eKYC provider selection** | The eKYC / identity-verification provider is defined **by role — document OCR, liveness/selfie match, and watchlist (sanctions/PEP) screening — not by vendor name.** The concrete provider is a **procurement decision (TBD)**; no vendor is named in any spec, sequence diagram, schema field, error code, or integration story, which reference the role ("the eKYC provider") and neutral identifiers (`kyc_inquiry_id`, `/webhooks/kyc`). **ХУР/XYP state-register lookup is the compliant-alternative candidate** for identity verification, because biometric eKYC (face-match + liveness) is **legally unresolved** (CLAUDE.md blocking-question 2 — two law-firm readings of the Personal Data Protection Law would restrict biometric collection to enumerated state bodies, contradicting reported bank face-match practice). | Named **Persona** as the canonical vendor, with **Jumio** as a fallback candidate; the prior assertion that substituting the vendor "is an integration change, not a requirements change." | The US vendors (Persona/Jumio) are assumed and are not the Mongolian market — they are market-invalid (see project context), so no vendor is named and the provider stays role-defined and TBD pending procurement and the biometric-eKYC legal resolution. **The prior claim that vendor substitution was "an integration change, not a requirements change" was false:** vendor names had leaked into schema field names (`persona_inquiry_id`) and API error codes, so swapping providers required editing the requirements, not merely an integration. This migration makes those fields and error codes vendor-neutral **so that going forward** selecting or substituting the concrete provider can become an integration/procurement change rather than a requirements change. The decision's meaning is unchanged (KYC provider selection); only the named vendor is removed. |
...
| **DEC-11** | **Initial membership share price** | **One mandatory membership share at 10,000₮ (provisional, pending DEC-11 / legal) par value**, purchased during onboarding; non-withdrawable while membership is ACTIVE; redeemable at par on CLOSED (subject to bylaws). | $25 (Sprint 2) vs $5 (Sprint 3). | 10,000₮ is meaningful enough to fund guarantee pledges and signal commitment while remaining a low entry barrier; it is the figure tied to the onboarding flow spec. Final par value remains subject to legal/regulatory confirmation, but 10,000₮ is the provisional working value for all documents pending DEC-11 / legal. |
...
| **DEC-19** | **KYC status enum** | `KycStatus = NOT_STARTED \| IN_PROGRESS \| PENDING_REVIEW \| APPROVED \| REJECTED`. PENDING_REVIEW routes to the CAP-12.2 manual queue. | Ad-hoc phrases ("compliance clearance", "verified"). | Downstream onboarding stories and the eKYC-provider integration need one status vocabulary. |
...
| **Membership Share Account** | Non-withdrawable equity account holding membership share(s); dividend-bearing; par 10,000₮. | "Co-op Share Account", "Share Capital Registry" (as account name) |
...
| **eKYC provider** | The identity-verification provider — document OCR, liveness, screening; a procurement decision (TBD) per DEC-5. | any named vendor (Persona, Jumio); "the KYC vendor" as though one is named |
...
1. **Legal/regulatory confirmation of the 10,000₮ share par value and share-redemption terms (DEC-11)** — requires counsel review against the chartering jurisdiction's cooperative/credit-union statute.
