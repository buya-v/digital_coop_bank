...
* **Actors:** Members are "Member A", "Member B", etc. Back-office staff (P-5) are "Operator A" (maker) and "Operator B" (checker). No placeholder personal names are used as actor identities; illustrative Cyrillic name *values* (e.g. DEC-6 field examples `ner` "Болд", and the DEC-35 recipient short-form display "Ц. Бат") are format examples, not actors.
