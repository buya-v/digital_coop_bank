...
| **Sponsor bank / BaaS provider** (e.g., a Unit-class platform over an FDIC/NCUA-insured partner institution — final selection is a procurement decision, not a requirements change) | FBO account structure, ACH origination/receipt, wire, real-time rails (FedNow), settlement of card network activity. |
