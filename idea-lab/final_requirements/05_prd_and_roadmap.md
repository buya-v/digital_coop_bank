...
Members join in minutes through fully digital eKYC onboarding, buy one $25 membership share, and immediately hold three things no neobank gives them: **a vote** (one member, one vote, from their phone, with Proxy Delegation), **a dividend** (an annual Patronage Dividend they can watch grow in real time), and **verifiable transparency** (a Transparent Capital Ledger showing exactly where the cooperative's collective funds are deployed).
...
**AC themes (03 EP-7, 17 scenarios):** sum of entitlements = distributable pool (hard invariant); factor weights are FINANCIAL_POLICY-governed configuration; estimator figures labeled "estimated" (DEC-15) with scenario bands; failed postings retried and reconciled; zero-activity members get a $0.00 statement, not an error.
...
| DEC-31 | EP-3 | Goal target > $0.00; target date, when set, in the future | **CONFIRM** | As proposed | Basic validation; no counter-case. |
...
| DEC-33 | EP-4 | Default P2P daily velocity limit $2,000.00/sender | **CONFIRM** | $2,000.00/day seed | Sensible launch AML posture; per-member exceptions and revisions live in US-12.5. |
...
| DEC-36 | EP-4 | Default step-up threshold for external payments: single transfer > $1,000.00 | **CONFIRM** | > $1,000.00 seed | Reasonable friction line: routine bills pass, account-draining transfers get challenged. |
...
| DEC-45 | EP-6 | Launch loan bounds min $100.00 / **max $1,000.00** | **AMEND** | Min $100.00 / **max $5,000.00** seed | A $1,000 cap (a de-ratified Sprint 1 draft) cannot serve P-3 equipment micro-loans, the 75–85% deposit-to-loan target (KPI-2.3), or a meaningful Loan Circle proposition; $5,000 stays prudent for an unproven book. |
...
| DEC-56 | EP-7 | Zero-patronage members receive a $0.00 entitlement statement (not an error) | **CONFIRM** | As proposed | Every member gets an explainable statement; reinforces reconciliation invariant. |
...
| DEC-73 | EP-13 | Seed AML scenario: aggregate outbound ≥ $10,000/24 h, or structuring patterns, raise an alert | **CONFIRM** | As proposed (seed only) | Correct illustrative seed pending the compliance officer's production rule set; ownership stays with compliance. |
...
* **Exit criteria:** Persona sandbox pass/review/fail paths drive `KycStatus` per DEC-19; $25.00 Stripe settlement transitions `PENDING_PAYMENT → ACTIVE` and opens both accounts; KPI-1.1 timestamps emitted; illegal DEC-4 transitions rejected; every action above appears in the tamper-evident audit log; eligibility snapshot API returns the ACTIVE set.
...
6. **Loan cap re-introduction:** `02`'s reconciliation notes de-ratified Sprint 1's $1,000 cap, yet `03` EP-6 re-proposed it as a seed → adjudicated to $5,000 (DEC-45).
