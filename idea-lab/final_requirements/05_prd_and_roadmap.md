...
| R-2 | **BaaS / sponsor-bank dependency:** the entire money-rail stack (FBO structure, ACH+/Banksüljee RTGS, NETC card settlement) rides one procurement decision (`04` §4.3); onboarding lead times and sponsor compliance reviews are notoriously long | High / High | Procurement must conclude **before S2** (US-4.2); daily three-way reconciliation and degradation modes are specified; the payment-processor path keeps share purchase (S1) independent of the BaaS timeline; abstract contracts in `04` make substitution "an integration change, not a requirements change". |
